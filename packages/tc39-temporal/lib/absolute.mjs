...
export class Absolute {
  ...
  getEpochSeconds() {
    ...
    return value.divide(1e9).value;
  }
  getEpochMilliseconds() {
    if (!ES.IsAbsolute(this)) throw new TypeError('invalid receiver');
    const value = bigInt(GetSlot(this, EPOCHNANOSECONDS));
    return value.divide(1e6).value;
  }
  ...
  plus(durationLike) {
    ...
    let add = bigInt(0);
    add = add.plus(bigInt(GetSlot(duration, NANOSECONDS)).multiply(1));
    ...
    add = add.plus(
      bigInt(GetSlot(duration, MINUTES))
        .multiply(60)
        .multiply(1e9)
    );
    add = add.plus(
      bigInt(GetSlot(duration, HOURS))
        .multiply(60 * 60)
        .multiply(1e9)
    );
    add = add.plus(
      bigInt(GetSlot(duration, DAYS))
        .multiply(24 * 60 * 60)
        .multiply(1e9)
    );
